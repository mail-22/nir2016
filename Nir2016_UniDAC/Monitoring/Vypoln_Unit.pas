...
begin
  Vypoln_Show('1.');
//1. –ешений по генеральному плану
end;
...
begin
  ...
  try
    ...
    RichEdit.Lines.Clear;
                //RichEdit.Lines.Append('ƒобавл€ема€ строка');
    s1 := ''; NNBilding := 0;
  ...
  //for i := 0 to DataSetMaster.RecordCount - 1 do
    begin
      s1 := '«дание N' + IntToStr(NNBilding + 1);
...
begin
  // Export в Microsoft Office Excel текущего объекта (здани€)
  inherited;
...
begin
  inherited;
  DM.tblVypoln.Close;
  //dm.tblVypoln.ParamByName('є').Value := '*';
...
//состо€ние таблицы
  cxgrdbtblvw1.RestoreFromIniFile(AFileName); //   восстанавливать состо€ние таблицы
...
// cxgrdbtblvw1 таблица
  ...
  cxGridDBTableView3.RestoreFromIniFile(AFileName); //   восстанавливать состо€ние табл
{
  // cxgrdbtblvw1 таблица
  ...
  cxGridDBTableView1.RestoreFromIniFile(AFileName); //   восстанавливать состо€ние таблицы
}
...
//состо€ние таблицы
  if FileExists(AFileName) then
    cxgrdbtblvw1.StoreToIniFile(AFileName, true); // сохран€ть состо€ние таблицы

  if FileExists(AFileName) then
    cxGridDBTableView3.StoreToIniFile(AFileName, true); // сохран€ть состо€ние таблицы
{
  if FileExists(AFileName) then
    cxGridDBTableView1.StoreToIniFile(AFileName, true); // сохран€ть состо€ние таблицы
}
